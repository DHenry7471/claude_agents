...
**File:** `agents/percy-pr-reviewer.md` | **Color:** pink | **Model:** haiku | **Tools:** Read, Glob, Grep, Bash
...
This repo runs its own CI gate (`.github/workflows/repo-quality-gate.yml`) on every PR and on
push to `main`:

| Job | What it catches |
|-----|-----------------|
| `build` | `cd mcp && pnpm run build` — a broken `bundle-prompts.mjs` run or TypeScript error |
| `test` | `cd mcp && pnpm run test` — regressions in `runAgent()`/`runHorusAgent()`/`registry.ts` |
| `lint-agents` | `node scripts/lint-agents.mjs` — an agent whose own instructions require a tool (`Write`, `Bash`, …) its frontmatter doesn't grant |

Run any of these locally the same way CI does. `lint-agents.mjs` is a lightweight heuristic
(no YAML dependency), not a full static analyzer — it exists to catch the specific class of bug
where an agent's `tools:` list and its own body instructions have drifted apart.
...
CHANGELOG.md                  # Version history for the published package
scripts/
  lint-agents.mjs             # Lints agents/**/*.md: declared tools vs. tools instructions require
...
mcp/                           # Published as @wutangbanger/claude-agents
  ...
  src/
    ...
    api.test.ts                # Unit tests for api.ts (Anthropic SDK mocked)
    registry.ts               # Lookup/list + SLUG_ALIASES (includes horus-* aliases)
    registry.test.ts           # Unit tests for registry.ts
